...
public class TableauStrategy implements PileStrategy {
	...
	public boolean checkCanDrop(Pile tableau, Packet packet) {
		// Bottom card of packet must have opposite color and one-less value of top
		...
		if (tableau.isEmpty()) {
			...
			Card bottom = packet.getBottom();
			return top.isFaceUp() && (top.getSuit().isRed() != bottom.getSuit().isRed())
					&& (top.getRank().getValue() - 1 == bottom.getRank().getValue());
		}
	}
	...
	public void handleClick(Pile tableau, MouseEvent event) {
		// Do nothing (future extension -- search for a place to move the top card)
	}

	@Override
	public void finishDrag(Pile tableau, Packet packet, Pile target, MouseEvent event) {
		// Flip over an exposed top card, if any
		if (!tableau.isEmpty() && !tableau.getTop().isFaceUp()) {
