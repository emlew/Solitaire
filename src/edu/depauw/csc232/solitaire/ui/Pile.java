...
public class Pile extends CardStack {
	private Pile(CardCollection cards, PileStrategy strategy, int horizontal, int vertical) {
	 ...
	 * @param strategy
	 */
	public static Pile makeHorizontal(CardCollection cards, PileStrategy strategy) {
	...
	public boolean underMouse(MouseEvent event) {
		...
		int ex = event.getX();
		int ey = event.getY();
		return image != null && x <= ex && ex < x + image.getWidth(null) && y <= ey && ey < y + image.getHeight(null);
	}
	 ...
	 * 
	 * @param packet the candidate for dropping on this pile
	 * @param event  the mouseMoved event being checked
	 ...
	 * 
	 * @param event the mouseDragged event being checked
	 * @return true if this pile will respond to being dragged
	 */
	public boolean canDrag(MouseEvent event) {
		// true if there is at least one card to be dragged, and the strategy says OK
	 ...
	 * 
	 * @param event the initial mouseDragged event recognized as a drag
	...
	public Packet startDrag(MouseEvent event) {
		...
		Packet packet = new Packet(this, selectedCards, xOFFSET / HOFFSET, yOFFSET / VOFFSET);
